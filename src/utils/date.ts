...
const getFractionalDay = (serial: number) => serial - Math.floor(serial) + 0.0000001;
const getTotalSeconds = (fractionalDay: number) => Math.floor(SECONDS_PER_DAY * fractionalDay);
...
export const getAllSerialDatesByMonth = (year: number, month: number): number[] => {
  ...
  const serialDates = Array.from({ length: lastSerial - firstSerial + 1 }, (_, i) => firstSerial + i);
