...
const allowedAccounts: string[] = process.env.NEXT_PUBLIC_ALLOWED_ACCOUNTS?.split(',') ?? [];

const accountMap: { [key: string]: Name } = {
  [allowedAccounts[0]]: 'wanny',
  [allowedAccounts[1]]: 'moomin',
};
...
const NameButtonGroup: React.FC = () => {
  ...
  const nameLeftButtonClass = (nameParam: string) =>
    `flex-1 px-4 py-2 text-sm font-medium border-t-2 border-b-2 border-l-2 border-r border-gray-200 rounded-l-lg ${
      name === nameParam ? 'bg-[#024280] text-white' : 'bg-white text-black hover:bg-gray-100'
    }`;
  ...
  return (
    <div className="flex" role="group">
      <button type="button" value="wanny" className={nameLeftButtonClass('wanny')} onClick={handleClick}>
        🐶 빵떡
      </button>
      <button type="button" value="moomin" className={nameRightButtonClass('moomin')} onClick={handleClick}>
        🐻‍❄️ 무민
      </button>
    </div>
  );
